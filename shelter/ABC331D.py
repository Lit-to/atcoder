...
class prefix_sum():
    ...
    def get_sum(self, left: int, right: int):
        ...
        Returns:
            -  int: 総和の結果
        """
        return self.__data[right+1] - self.__data[left]
...
def main():
    ...
    def funcG(h,w):
        partA_h=h//N
        partA_w=w//N
        a = blackCount*partA_h*partA_w

        partB_h=h%N
        b = psRow.get_sum(0,partB_h)*partA_w

        partC_w=w%N
        c = psColumn.get_sum(0,partC_w)*partA_h

        d = 0
        for i in range(partB_h):
            for j in range(partC_w):
                d+=BOARD[i,j]=="B"
        return a+b+c+d
        ...
    ...
    N, Q = split(input(),func=int)
    BOARD=Board.input_board(N)
    QUERIES =[]
    for i in range(Q):
        QUERIES.append(split(input(),func=int))
    ...
    ...
    for i in range(H):
        for j in range(W):
            blackCount+=BOARD[i,j]=="B"

    binBoard = []
    for i in range(H):
        line = []
        for j in range(W):
            line.append(BOARD[i,j]=="B")
        binBoard.append(line)
    row = []
    for i in range(H):
        row.append(sum(binBoard[i]))
    psRow = prefix_sum(row)
    ...
    column = []
    for i in range(H):
        column.append(sum(binBoard[i]))
    psColumn = prefix_sum(column)
    BOARD.rotate(3)

    for i in range(Q):
        A,B,C,D = QUERIES[i]
        print(funcG(C,D+1)-funcG(C,B+1)-funcG(A,D+1)+funcG(A,B+1))
