...
void solve()
{
    ...
    */
    ...
    for (ll i = 0; i < N; ++i)
    {
        cin >> sections[i].l >> sections[i].r >> sections[i].h;
        --sections[i].l;
    }
    const ll INF = 24;
    std::vector<ll> days(D, INF);
    for (ll i = 0; i < N; ++i)
    {
        for (ll l = sections[i].l; l < sections[i].r; ++l)
        {
            Lit::ChangeMin(days[l], sections[i].h);
        }
    }
    const ll result = Lit::Sum(days);
    cout << result << endl;
}
