...
void solve()
{
    ...
    */
    ...
    for (ll i = 0; i < N; ++i)
    {
        cin >> people[i].HEALTH >> people[i].MENTAL;
    }
    std::sort(all(people));
    ll hRight = 0;
    ll result = 0;
    std::vector<PERSON> mentalPeople(N);
    for (ll hLeft = 0; hLeft < N; ++hLeft)
    {
        while (hRight < N && people[hRight].HEALTH <= people[hLeft].HEALTH + K)
        {
            std::copy(all(people), mentalPeople.begin());
            std::sort(mentalPeople.begin() + hLeft, mentalPeople.begin() + hRight + 1, [&](PERSON &l, PERSON &r)
                      { return l.MENTAL < r.MENTAL; });

            ll mRight = hLeft; // 初期値 0の地点
            for (ll mLeft = hLeft; mLeft < hRight + 1; ++mLeft)
            {
                while (mRight < hRight + 1 && mentalPeople[mRight].MENTAL <= mentalPeople[mLeft].MENTAL + K)
                {
                    Lit::ChangeMax(result, mRight + 1 - mLeft);
                    ++mRight;
                }
                if (mLeft == mRight)
                {
