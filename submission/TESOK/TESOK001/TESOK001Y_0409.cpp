...
int main()
{
    ...
    for (int64_t i = 0; i < H; ++i)
    {
        for (int64_t j = 0; j < W; ++j)
        {
            for (int k = 0; k < 2; ++k)
            {
                int64_t diff_y = LRUD[k][0];
                int64_t diff_x = LRUD[k][1];
                int64_t nextY = diff_y + i;
                int64_t nextX = diff_x + j;
                if (BOARD.IsInside(nextY, nextX) && BOARD[nextY, nextX] == '.')
                {
                    result[diff_y + i, diff_x + j] += result[i, j];
                }
            }
        }
    }
    std::cout << result[H - 1, W - 1] << std::endl;
}
