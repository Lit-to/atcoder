...
template <typename T>std::vector<T> input(int64_t n){std::vector<T> contents(n);for (int64_t i = 0; i < n; ++i){contents[i] = input<T>()-1;}return contents;}
// clang-format on
#include <vector>
#include <cstdint>
#include <iostream>
...
void solve()
{
    ...
    */
    ...
    std::vector<NEST> nests(Q);
    for (ll i = 0; i < Q; ++i)
    {
        cin >> nests[i].nest >> nests[i].day;
    }
    const ll W = 31;
    Board<ll> DP(N, W + 1);
    for (ll i = 0; i < N; ++i)
    ...
    for (ll i = 0; i < Q; ++i)
    {
        ll nest = nests[i].nest - 1;
        ll day = nests[i].day;
        auto moveArr = Lit::GenerateBit(day, W);
        std::reverse(all(moveArr));
        for (ll j = 0; j < W; ++j)
        {
            if (moveArr[j])
            {
