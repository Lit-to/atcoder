...
void solve()
{
    ...
    cin >> N;
    cin >> T;
    vector<ll> beautiful(N + 1);
    vector<ll> nonBeautiful(N + 1);
    vector<bool> isBeautiful(N + 1);
    for (ll i = 0; i < N; ++i)
    {
        isBeautiful[i + 1] = T[i] == '1';
    }
    for (ll i = 1; i < N + 1; ++i)
    {
        if (isBeautiful[i])
        {
            beautiful[i]++;
            beautiful[i] += beautiful[i - 1];
            nonBeautiful[i] += nonBeautiful[i - 1];
        }
        else
        {
            nonBeautiful[i]++;
            beautiful[i] += nonBeautiful[i - 1];
            nonBeautiful[i] += beautiful[i - 1];
        }
    }
    cout << LitM::Sum(beautiful) << endl;
}
