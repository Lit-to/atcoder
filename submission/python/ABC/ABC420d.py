...
def main():
    ...
    # def dfs(pos,count,done):
    #     i,j,d = pos
    #     for k,l in LRUD:
    #         next=BOARDS[d][(i+k,j+l)]
    #         if next==".":
    #             next_pos =(i+k,j+l,d)
    #             if next_pos not in done:
    #                 done.add(next_pos)
    #                 dfs(next_pos,count+1,done)
    #                 done.remove(next_pos)
    #         elif next == "?":
    #             next_pos =(i+k,j+l,1-d)
    #             if next_pos not in done:
    #                 done.add(next_pos)
    #                 dfs(next_pos,count+1,done)
    #                 done.remove(next_pos)
    ...
    BOARDS = []
    BOARDS.append(Board.input_board_with_wall(H,W,"#"))
    board = BOARDS[0].copy()
    START = (0,0)
    for i in range(H):
        for j in range(W):
