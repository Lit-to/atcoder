...
void solve()
{
    ...
    vector<ll> preSumV(N + 1);
    vector<ll> preSumF(N + 1);
    for (ll i = 0; i < N; ++i)
    {
        preSumF[i + 1] = preSumF[i] + (S[i] == 'F' || S[i] == 'B');
        preSumV[i + 1] = preSumV[i] + (S[i] == 'V' || S[i] == 'B');
    }
    auto funcT = [&](ll pos) -> auto
    { return preSumF[pos] - preSumV[pos]; };
