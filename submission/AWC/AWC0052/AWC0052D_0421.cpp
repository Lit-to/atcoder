...
void solve()
{
    ...
    std::vector<SCAN_RESULT> scanResult(M);
    for (int64_t i = 0; i < M; ++i)
    {
        std::cin >> scanResult[i].K;
        scanResult[i].TARGETS.resize(scanResult[i].K);
        for (int64_t j = 0; j < scanResult[i].K; ++j)
        {
            std::cin >> scanResult[i].TARGETS[j];
            --scanResult[i].TARGETS[j];
            scanResult[i].targetBits += 1 << scanResult[i].TARGETS[j];
        }
        std::cin >> scanResult[i].R;
    }
    ...
    for (int64_t bits = 0; bits < 1 << N; ++bits)
    {
        ...
        for (int64_t i = 0; i < M; ++i)
        {
            int64_t judge = bits & scanResult[i].targetBits;
            if ((scanResult[i].R == 1) != (0 < judge))
            {
                isCorrect = false;
                break;
            }
        ...
        std::vector<bool> boolBits = Lit::GenerateBit(bits, N);
        int64_t candidate = 0;
        for (int64_t i = 0; i < N; ++i)
        {
            candidate += boolBits[i];
        }
        Lit::ChangeMin(result, candidate);
    }
