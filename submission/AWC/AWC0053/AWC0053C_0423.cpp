...
void solve()
{
    ...
    {
        std::cin >> wifi[i].X >> wifi[i].L >> wifi[i].R >> wifi[i].C;
    }
    struct BORDER
    {
        int64_t index;
         ...
         * ソート用の比較関数
         */
        bool operator<(const BORDER &target) const
        {
            return pos < target.pos || pos == target.pos && index < target.index;
        }
    };
    std::vector<BORDER> B;
    int64_t count = 0;
    ...
    {
        B.push_back(BORDER{
            .index = ++count, .pos = wifi[i].X - wifi[i].L, .strength = wifi[i].C});
        B.push_back(BORDER{
            .index = ++count, .pos = wifi[i].X + wifi[i].R + 1, .strength = -wifi[i].C});
    }
    std::sort(B.begin(), B.end());
    std::vector<int64_t> PFS(1);
    for (int64_t i = 0; i < 2 * N; ++i)
    {
        PFS.push_back(PFS.back() + B[i].strength);
    }
    std::cout << Lit::Max(PFS) << std::endl;
}
