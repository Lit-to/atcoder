...
def main():
    ...
    N = im.intInput()
    S = im.input()
    diffAB = dict(lambda:0)
    diffBC = dict(lambda:0)
    diffCA = dict(lambda:0)
    diffALL = dict(lambda:0)
    ABC = dict(lambda:0)
    diffAB[0] = 1
    diffBC[0] = 1
    diffCA[0] = 1
    ...
    for i in range(N):
        ABC[(S[i])]+=1
        diffAB[ABC["A"]-ABC["B"]]+=1
        diffBC[ABC["B"]-ABC["C"]]+=1
        diffCA[ABC["C"]-ABC["A"]]+=1
        diffALL[(ABC["A"]-ABC["B"],ABC["A"]-ABC["C"])]+=1
    ...
    print(((N*(N+1))//2)-(calcSum(diffAB)+calcSum(diffCA)+calcSum(diffBC))+2*calcSum(diffALL))
