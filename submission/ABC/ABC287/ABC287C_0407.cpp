...
int main()
{
    ...
    auto dfs = [&](auto &&self, int64_t prev, int64_t node) -> void
    {
        done[node] = true;

        if (3 <= GRAPH[node].size())
        {
            Lit::No();
            exit(0);
        }
        ...
        for (int64_t i = 0; i < GRAPH[node].size(); ++i)
        {
            ...
            // done[GRAPH[node][i]] = true;
            return self(self, node, GRAPH[node][i]);
        };
    };
    if (3 <= GRAPH[0].size())
    {
    ...
    for (int64_t i = 0; i < GRAPH[0].size(); ++i)
    {
        dfs(dfs, 0, GRAPH[0][i]);
    }
    done[0] = true;
    for (int64_t i = 0; i < done.size(); ++i)
    {
        if (done[i] == false)
        {
            Lit::No();
