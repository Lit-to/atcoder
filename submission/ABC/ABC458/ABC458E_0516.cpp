...
void solve()
{
    ll A, B, C;
    cin >> A >> B >> C;
    const ll DIGITS = A + B + C;
    const vector<ll> LIMITS({A, B, C});
    // 値iをj個使ってk桁埋めたときの通り数
    const ll maxCount = LitM::Max(LIMITS);
    vector<vector<vector<mint>>> DP(3, vector<vector<mint>>(maxCount + 1, vector<mint>(DIGITS)));
    if (0 < A)
    {
        DP[0][0][0] = mint(1);
    };
    ...
    ll result = (DP[0][maxCount][DIGITS] + DP[1][maxCount][DIGITS - 1] + DP[2][maxCount][DIGITS - 1]).val();
    cout << result << endl;
}
