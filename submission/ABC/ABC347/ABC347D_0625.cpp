...
bool resolve(ll a, ll b, ll c, ll A, ll B)
{
    assert((0 <= a && a <= 1LL << 60));
    assert((0 <= b && b <= 1LL << 60));
    assert(Lit::Sum(Lit::GenerateBit(a, 60)) == A);
    assert(Lit::Sum(Lit::GenerateBit(b, 60)) == B);
    assert((a ^ b) == c);
    return true;
}
...
void solve()
{
    ...
    ll A_ = A, B_ = B, C_ = C;
    const ll N = 60;
    auto cVector = Lit::GenerateBit(C_, N);
    ll cMin = Lit::Sum(cVector);
    ll diff = A_ + B_ - cMin;
    if (diff < 0 || diff % 2 != 0 || (N - cMin) < diff / 2)
    {
        cout << -1 << endl;
        return;
    }
