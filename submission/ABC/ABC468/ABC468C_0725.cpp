...
void solve()
{
    ...
    */
    ...
    const auto Q = input<ll>(N);
    std::vector<ll> perm(N);
    for (ll i = 0; i < N; ++i)
    {
        perm[i] = i + 1;
    }
    auto eval = [&](const std::vector<ll> &expectSmall, const std::vector<ll> &expectBig)
    {
        ll index = 0;
        while (index < N - 1 && expectBig[index] == expectSmall[index])
        {
            ++index;
        }
        return (expectSmall[index] < expectBig[index]);
    };
    ll result = 0;
    do
    {
        bool isOK = true;
        isOK = isOK && eval(P, perm);
        isOK = isOK && eval(perm, Q);
        result += isOK;
