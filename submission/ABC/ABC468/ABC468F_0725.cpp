...
void solve()
{
    ...
    */
    ...
    const auto P = input<ll>(10);
    // i番目の操作で操作jを選んだときのcの最大値
    Board<ll> DP(N + 1, 2);
    DP[0, 0] = 0;
    DP[1, 0] = 0;
    ll xmin = 0; // i番目の操作までのxの最大値
    ll ymin = 0; // i番目の操作までのyの最大値
    for (ll i = 0; i < N; ++i)
    {
        DP[i];
        if (xmin < P[i])
        {
            DP[i + 1, 0] = std::max(DP[i, 0], DP[i, 1]) + 1;
            xmin = std::min(xmin, std::max(xmin, P[i]));
        }
        if (ymin < P[i])
        {
            DP[i + 1, 0] = std::max(DP[i, 0], DP[i, 1]) + 1;
            ymin = std::min(ymin, std::max(ymin, P[i]));
        }
    }
    cout << DP[1, N] << std::endl;
}
