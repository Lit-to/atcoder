    ...
    */
    ...
    for (ll i = 0; i < H; ++i)
    {
        for (ll j = 0; j < W; ++j)
        {
            ...
            {
                queue.push(POS{.X = j, .Y = i, .K = 0});
                isDone.insert(BOARD.ConvertPosToIndex(i, j));
            }
        }
    }
    ...
    while (K != 0 && !queue.empty())
    {
        ...
        for (auto &d : LRUD)
        {
            ll distY = d[0] + y;
            ll distX = d[1] + x;
            ll index = BOARD.ConvertPosToIndex(distY, distX);
            if (!BOARD.IsInside(distY, distX))
            {
                // 場外ホームラン
                continue;
            }
            else if (isDone.contains(index))
            {
                // もう行った
                continue;
            }
            else if (BOARD[distY, distX] == '#')
            {
                // 移動不可マス
                continue;
            }
            // std::cerr << distY << " " << distX << endl;
            isDone.insert(index);
            if (K <= k + 1)
            {
                continue;
            }
            queue.push(POS{.X = distX, .Y = distY, .K = k + 1});
        }
    }
    ll result = isDone.size();
    cout << result << endl;
}
