...
void solve()
{
    ...
    */
    ...
    std::vector<ll> maru(N);
    std::vector<ll> batu(N);
    for (ll i = 0; i < N; ++i)
    {
        if (S[i] == 'o')
        {
    ...
    PrefixSumManager preMaru(maru);
    PrefixSumManager preBatu(batu);
    for (ll i = 0; i < N; ++i)
    {
        ll life = preMaru.GetSum(0, i + 1);
        ll maxIndex = Lit::Search(N - 1, i - 1, [&](ll index)
                                  { return life <= preBatu.GetSum(i + 1, index + 1); });
