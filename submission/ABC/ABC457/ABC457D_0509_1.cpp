...
void solve()
{
    ...
    struct NUM
    {
        ll value;
        ll index;
        bool operator>(const NUM &target) const
        {
            return value != target.value ? value > target.value : index > target.index;
        }
    };
    greater_priority_queue<NUM> gpq;
    for (ll i = 0; i < N; ++i)
    {
        gpq.push(NUM{.value = A[i], .index = i + 1});
    }
    ll count = 0;
    while (count < K)
    {
        NUM target = gpq.top();
        gpq.pop();
        ll m = LitA::Search(K + 1, 0, [&](ll mul)
                            { return gpq.top().value <= target.value + target.index * mul; });
        if (K <= count + m)
        {
            m = K - count;
        }
        target.value += target.index * m;
        count += m;
        gpq.push(target);
    }
    cout << gpq.top().value << endl;
}
