...
void solve()
{
    ...
    cin >> N;
    ll p = 0;
    std::vector<ll> pairCount(N, 1);
    for (ll i = 0; i < N; ++i)
    {
        ll index = Lit::Search(i, N, [&](ll index)
                               { return query(i, index); });
        pairCount[i] = index - i + 1; // 自分自身を含める
    }

    ll result = 0;
    for (ll i = 0; i < pairCount.size(); ++i)
    {
        result += (pairCount[i] * (pairCount[i] - 1)) / 2;
    }
    cout << "! " << result << endl;
}
