...
void solve()
{
    ...
    */
    ...
    vector<linkedListNode> linkedList(n + 2);
    ll posIndex = 0;
    for (ll i = 0; i < n + 2; ++i)
    {
        linkedList[i].next = i + 1;
        linkedList[i].prev = i - 1;
        linkedList[i].value = INF;
    }

    linkedList[0].value = -1 * INF;
    for (ll i = 0; i < n; ++i)
    {
        linkedList[i + 1].value = a[i];
        if (a[i] == 0)
        {
            posIndex = i + 1;
        }
    }
    ll size = n - 1; // 0を1個取った状態
    ll result = 0;
    while (0 < size)
    {
        ll posValue = linkedList[posIndex].value;
        auto right = linkedList[linkedList[posIndex].next].value;
        auto left = linkedList[linkedList[posIndex].prev].value;
        ll rightDiff = abs(right - posValue);
        ll leftDiff = abs(left - posValue);
        ll dest;
        if (leftDiff <= rightDiff)
        {
            dest = linkedList[posIndex].prev;
        }
        else
        {
            dest = linkedList[posIndex].next;
        }
        result += abs(posValue - linkedList[dest].value);

        linkedList[linkedList[posIndex].next].prev = linkedList[posIndex].prev;
        linkedList[linkedList[posIndex].prev].next = linkedList[posIndex].next;
