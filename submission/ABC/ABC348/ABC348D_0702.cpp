...
void solve()
{
    ...
    vector<POS> energies(N);
    for (ll i = 0; i < N; ++i)
    {
        cin >> energies[i].Y >> energies[i].X >> energies[i].energy;
        --energies[i].Y;
        --energies[i].X;
    }
    ll startPos[] = {0, 0};
    ...
    vector<vector<Dijkstra::EDGE>> GRAPH(H * W);
    Board<ll> BOARD_MED(H, W);
    BOARD_MED.Fill(-1);
    for (ll i = 0; i < N; ++i)
    {
        BOARD_MED[energies[i].Y, energies[i].X] = i;
    }

    for (ll i = 0; i < N; ++i)
    {
        queue<TASK> tasks;
        tasks.push(TASK{.X = energies[i].X, .Y = energies[i].Y, .energy = energies[i].energy, .from = BOARD.ConvertPosToIndex(energies[i].Y, energies[i].X)});
        ...
        while (!tasks.empty())
        {
            ...
            tasks.pop();
            done[BOARD.ConvertPosToIndex(task.Y, task.X)] = true;
            if (0 < task.energy)
            {
                for (ll j = 0; j < 4; ++j)
                {
                    ll newPos[] = {task.Y + LRUD[j][0], task.X + LRUD[j][1]};
                    const ll boardIndex = BOARD.ConvertPosToIndex(newPos[0], newPos[1]);
                    if (!BOARD.IsInside(newPos[0], newPos[1]))
                    {
                        continue;
                    }
                    if (done[boardIndex])
                    ...
                    done[boardIndex] = true;
                    ll energyIndex = BOARD_MED[boardIndex];
                    if (0 <= energyIndex)
                    {
                        ll posTo = BOARD.ConvertPosToIndex(energies[energyIndex].Y, energies[energyIndex].X);
                        GRAPH[task.from].push_back(Dijkstra::EDGE{.to = posTo, .cost = 1});
                    }
                    if (newPos[0] == goalPos[0] && newPos[1] == goalPos[1])
                    {
                        GRAPH[task.from].push_back(Dijkstra::EDGE{.to = BOARD.ConvertPosToIndex(goalPos[0], goalPos[1]), .cost = 1});
                    }
                    tasks.push(TASK{.X = newPos[1], .Y = newPos[0], .energy = task.energy - 1, .from = task.from});
                }
            }
        }
    }
    Dijkstra dijk(GRAPH);
    auto result = dijk.calcCosts(BOARD.ConvertPosToIndex(startPos[0], startPos[1]));
    Lit::YesNo(result[BOARD.ConvertPosToIndex(goalPos[0], goalPos[1])] <= 1e9);
}
