...
def main():
    ...
    result = []
    for i in range(N):
        if not(abs(len(S[i])-len(T))<=1):
            continue
        elif S[i]==T:
            result.append(i+1)
            continue
        elif conditionB(max(len(S[i]),len(T)),S[i],T):
            result.append(i+1)
            continue
        elif len(S[i])==len(T) and conditionD(len(T),S[i],T):
            result.append(i+1)
            continue
    print(len(result))
    print(*result)
    ...
    if sep == "":
        ...
        for i in range(len(value)):
            if value[i] == sep:
                ...
                section = i
        if i == 0 or section != i:
            result.append(value[section:].split()[0])
