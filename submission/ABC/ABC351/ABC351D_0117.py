...
def main():
    ...
    BOARD:Board = Board.input_board(H)
    BOARD.add_wall("%")
    def calcPos(y,x):
        return y*W+x
    ...
    # 処理スペース
    uf:unionFind = unionFind(H*W)
    freeBoard:Board = Board.create_board(H,W,False)
    freeBoard.add_wall(False)
    for i in range(H):
        for j in range(W):
            if BOARD[i,j]=="#":
                continue
            for diffY,diffX in LRUD:
                if BOARD[i+diffY,j+diffX]=="#":
                    break
            else:
                freeBoard[i,j] = True
    ...
    for i in range(H):
        for j in range(W):
            if freeBoard[i,j]:
                for diffY,diffX in LRUD:
                    if freeBoard[i+diffY,j+diffX]:
                        uf.unite(calcPos(i,j),calcPos(i+diffY,j+diffX))

    d = dict(lambda:0)
    for i in range(H):
        for j in range(W):
            if freeBoard[i,j]:
                pos = uf.root(calcPos(i,j))
                d[pos] = uf.getSize(pos)
            else:
                d[calcPos(i,j)] = 1
    ...
    for i in range(H):
        for j in range(W):
            if freeBoard[i,j] == False and BOARD[i,j]!="#":
                movable = set()
                for diffY,diffX in LRUD:
                    if freeBoard[i+diffY,j+diffX]:
                        movable.add(uf.root(calcPos(i+diffY,j+diffX)))
                for k in movable:
                    d[k]=d[k]+1
...
class unionFind:
    ...
    def getSize(self,node):
        return self.par[self.root(node)]*-1
