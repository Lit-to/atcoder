...
int main()
{
    ...
    {
        std::string nowName, nextName;
        std::cin >> nowName;
        std::cin >> nextName;
        keys.push_back(nowName);
        GRAPH[nowName] = nextName;
    }
    std::map<std::string, bool> done;
    std::map<std::string, bool> startNode;

    auto dfs = [&](auto &&self, std::string &start, std::string &name) -> bool
    {
        if (startNode[name])
        {
            if (name == start)
        ...
        return self(self, start, GRAPH[name]);
    };
    for (int64_t i = 0; i < keys.size(); ++i)
    {
        ...
        if (Lit::No(!dfs(dfs, keys[i], keys[i])))
        {
            return 0;
        }
        startNode[keys[i]] = true;
    }
    Lit::Yes();
}
