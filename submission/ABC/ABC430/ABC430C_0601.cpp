...
void solve()
{
    ...
    PrefixSumManager psmA(a);
    PrefixSumManager psmB(b);
    ll ans = 0;
    for (ll l = 0; l < N; ++l)
    {
        // 1. a が A 個以上となる「最小の右端 r」を二分探索
        // [l, N] の範囲で、右に行くほど true になる (ng -> ok)
        ll ngA = l - 1;
        ll okA = N + 1;
        ll Aindex = LitUtility::Search(okA, ngA, [&](ll index)
                                       {
            if (index < l || index > N) return false;
            return A <= psmA.GetSum(l, index); });

        // a が A 個以上になる右端が存在しない場合はスキップ
        if (Aindex > N || !(A <= psmA.GetSum(l, Aindex)))
        {
            continue;
        }

        // 2. b が B 個未満となる「最大の右端 r」を二分探索
        // [l, N] の範囲で、左に行くほど true になる (ok -> ng)
        ll okB = l - 1; // 1文字も選ばない(r=l)とき、bは0個で必ずB未満(B>=1想定)
        ll ngB = N + 1;
        ll Bindex = LitUtility::Search(okB, ngB, [&](ll index)
                                       {
            if (index < l) return true;
            if (index > N) return false;
            return psmB.GetSum(l, index) < B; });

        // 条件を満たす r の範囲は [Aindex, Bindex]
        // 累積和の右端(半開区間)として、Aindex 以上 Bindex 以下の個数をカウント
        if (Aindex <= Bindex)
        {
            ans += (Bindex - Aindex + 1);
        }
