...
void solve()
{
    ...
    set<PERSON> POS;
    vector<ll> distance(N + 2);
    POS.insert(PERSON{.pos = 0, .index = 0});
    POS.insert(PERSON{.pos = INT64_MAX, .index = N + 1});
    distance[0] = INT64_MAX;
    // distance[N + 1] = (ll)1e9 + 1;
    ll result = LitM::Sum(distance);
    for (ll i = 0; i < N; ++i)
    {
        PERSON now = PERSON{.pos = X[i], .index = i + 1};
        POS.insert(now);
        auto prevIter = std::prev(POS.find(now));
        auto nowIter = POS.find(now);
        auto nextIter = std::next(POS.find(now));
        result -= distance[prevIter->index];
        result -= distance[nextIter->index];
        distance[prevIter->index] = std::min(distance[prevIter->index], nowIter->pos - prevIter->pos);
        distance[nowIter->index] = std::min(nowIter->pos - prevIter->pos, nextIter->pos - nowIter->pos);
        distance[nextIter->index] = std::min(distance[nextIter->index], nextIter->pos - nowIter->pos);
        result += distance[prevIter->index];
        result += distance[nowIter->index];
        result += distance[nextIter->index];
        cout << result << endl;
    }
}
