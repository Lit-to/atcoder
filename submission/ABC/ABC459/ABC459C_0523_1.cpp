...
void solve()
{
    ...
    vector<ll> num(N);
    // vector<ll> counts(3 * LitM::Pow(10, 5) + 1);
    ll mx = 3 * LitM::Pow(10, 5) + 1;
    SegmentTree<ll> countTree(mx, 0, [&](ll i, ll j)
                              { return i + j; });
    countTree.updateQuery(0, N);
    // counts[0] = N;
    ll offset = 0;
    for (QUERY &query : queries)
    {
        ...
        {
            --query.xy;
            countTree.updateQuery(num[query.xy], countTree.getQuery(num[query.xy], num[query.xy] + 1) - 1);
            ++num[query.xy];
            countTree.updateQuery(num[query.xy], countTree.getQuery(num[query.xy], num[query.xy] + 1) + 1);
            if (countTree.getQuery(num[query.xy], num[query.xy] + 1) == 0)
            {
                ++offset;
            }
        }
        ...
        {
            cout << countTree.getQuery(query.xy + offset, mx + 1) << endl;
        }
    }
}
