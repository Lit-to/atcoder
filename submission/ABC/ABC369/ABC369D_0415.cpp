...
int main()
{
    ...
    int64_t result = 0;
    Board<int64_t> DP(N, 2);
    DP.set(0, 1, A[0]);
    // 0=> 偶数 1=>奇数
    for (int64_t i = 1; i < N; ++i)
    {
        std::vector<int64_t> candidate(2);
        candidate[0] = DP.at(i - 1, 0) + A[i]; // 現在が奇数番目
        candidate[1] = DP.at(i - 1, 1);        // 現在が奇数番目(見逃し)
        DP.set(i, 1, Lit::Max(candidate));
        candidate[0] = DP.at(i - 1, 1) + A[i] + A[i]; // 現在が偶数番目
        candidate[1] = DP.at(i - 1, 0);               // 現在が偶数番目(見逃し)
        DP.set(i, 0, Lit::Max(candidate));
    }

    std::cout << std::max(DP.at(N - 1, 0), DP.at(N - 1, 1)) << std::endl;
}
