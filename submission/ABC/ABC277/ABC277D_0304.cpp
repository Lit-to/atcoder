...
int main()
{
    ...
    for (int i = 0; i < 2 * N; ++i)
    {
        int left = (i % N);
        int right = ((i + 1) % N);
        if (A[left] == A[right] || (A[left] + 1) % M == A[right] % M)
        {
            if (uf.IsSameRoot(left, right))
            {
                break;
            }
            uf.Unite(left, right);
    ...
    for (int i = 0; i < N; ++i)
    {
        int64_t root = uf.GetRoot(i);
        map[root] += A[i];
    }
    int64_t max = 0;
    int64_t asum = Sum(A);
    for (const std::pair<int, int64_t> &i : map)
    {
        max = std::max(max, i.second);
    }
    std::cout << asum - max << std::endl;
}