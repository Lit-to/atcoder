...
namespace LitMath
{
     ...
     * @brief Nの素因数分解の結果を返す
     */
    vector<int64_t> calcPrime(int64_t N)
    {
...
void solve()
{
    ...
    ll result = N * (N - 1) / 2 - (N - nums[0]) * (N - nums[0] - 1) / 2;
    for (ll val = 1; val < 210000; ++val)
    {
        result += nums[val] * (nums[val] - 1) / 2;
    }
    cout << result << endl;
}
