...
def main():
    ...
    def calc_h_candidate():
        if j-w<0:
            return dp[i][j] # 体に置く
        else:
            return dp[i][j-w]+PARTS[i][1]

    def calc_b_candidate():
        return dp[i][j]+PARTS[i][2]
    ...
    for i in range(N):
        line = []
        w = PARTS[i][0]
        for j in range(W_SUM):
            if W_SUM//2<j:
                break
            h_candidate = calc_h_candidate()
            b_candidate = calc_b_candidate()
            line.append(max(h_candidate,b_candidate))
        dp.append(line)
    print(dp[N][-1])
