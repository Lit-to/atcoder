...
void solve()
{
    ...
    Board<ll> DP(N + 1, 2); // i文字目を010101の並び(%2==0文字目のとき0)or1010101(%2==0文字目のとき1)の並びにしたときのコストの最小値
    // DP.Fill(Lit::Sum(C));
    // DP[0, 0] = 0;
    // DP[0, 1] = 0;
    for (ll i = 0; i < N; ++i)
    ...
    Board<ll> BOARD_A(N + 1, 2);
    Board<ll> BOARD_B(N + 1, 2);
    for (ll i = 0; i < N; ++i)
    {
        for (ll j = 0; j < 2; ++j)
        {
    ...
    for (ll i = 0; i < N + 1; ++i)
    {
        BOARD_A[i + 1, 0] += BOARD_A[i, 0];
    }
    for (ll i = N; 0 < i; --i)
    {
        BOARD_A[i - 1, 1] += BOARD_A[i, 1];
    }
    for (ll i = 0; i < N + 1; ++i)
    {
        BOARD_B[i + 1, 1] += BOARD_B[i, 1];
    }
    for (ll i = N; 0 < i; --i)
    {
        BOARD_B[i - 1, 0] += BOARD_B[i, 0];
    }
    cerr << BOARD_A.toString() << endl;
    cerr << "--" << endl;
    cerr << BOARD_B.toString() << endl;
    const ll INF = 1e18;
    ll result = INF;

    for (ll i = 0; i < N; ++i)
    {
        result = min(result, BOARD_A[i, 0] + BOARD_A[i, 1]);
        result = min(result, BOARD_B[i, 0] + BOARD_B[i, 1]);
    }
    cout << result << endl;
}
