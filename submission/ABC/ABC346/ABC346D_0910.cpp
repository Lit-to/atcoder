...
void solve()
{
    ...
    // std::reverse(all(cost10));
    vector<ll> preSum01(N + 1);
    vector<ll> preSum10(N + 1);
    for (ll i = 0; i < N; ++i)
    {
        preSum01[i + 1] = preSum01[i] + cost01[i];
        preSum10[i + 1] = preSum10[i] + cost10[i];
    }
    const ll INF = 1e18;
    ll result = INF;
    // std::reverse(all(preSum10));
    for (ll i = 1; i < N; ++i)
    {
        Lit::ChangeMin(result, preSum01[N] - preSum01[i] + preSum10[i]);
        Lit::ChangeMin(result, preSum01[i] + preSum10[N] - preSum10[i]);
    }
    cout << result << endl;
}
