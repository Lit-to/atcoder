...
void solve()
{
    ...
    const auto S = input<ll>();
    ll s = S - 1;
    const auto L = input<ll>(); // 最大1e18とか言ってるが実質6.4*10^6とかにしかならないねこれ
    const auto A = input<ll>(N - 1);
    auto preSum = PrefixSumManager(A);
    ll result = 1;
    for (ll l = 0; l < N; ++l)
    {
        ...
        for (ll r = l; r < N; ++r)
        {
            ...
            ll lCost = preSum.GetSum(l, s);
            ll rCost = preSum.GetSum(s, r);
            ll cost = lCost + rCost + std::min(lCost, rCost);
            if (cost <= L)
            {
                Lit::ChangeMax(result, r + 1 - l);
            }
        }
    }
    cout << result << endl;
