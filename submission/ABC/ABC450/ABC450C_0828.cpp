...
void solve()
{
    ...
    */
    ...
    std::unordered_set<ll> fillBlackDone;
    const int64_t LRUD_4[4][2] = {{0, -1}, {-1, 0}, {0, 1}, {1, 0}};

    /**
     * DFSスニペット
     */
    auto fillBlackDFS = [&](auto self, ll posY, ll posX) -> void // 引数にノード、戻り値は適度に
    {
        auto index = BOARD.ConvertPosToIndex(posY, posX);
        // 訪問済み管理のチェック
        if (fillBlackDone.contains(index))
        {
            return;
        }
        if (BOARD[posY, posX] == '#')
