...
void solve()
{
    ...
    auto isEqual = [&](ll s_indexL, ll t_indexL, ll distance) -> bool
    {
        for (ll i = 0; i < distance; ++i)
        {
            if (S[s_indexL + i] == T[t_indexL + i])
            {
                continue;
            }
            else
    ...
    vector<ll> rawImos(NS);
    for (ll i = 0; i < NS - NT + 1; ++i)
    {
        if (isEqual(i, 0, NT))
        {
            ++rawImos[i];
    ...
    for (ll i = 0; i < NS; ++i)
    {
        imos[i + 1] = imos[i] + rawImos[i];
    }
    for (ll i = 0; i < Q; ++i)
    {
        ll l = queries[i].L;
        ll r = queries[i].R - NT + 1;
        if (!(l < r))
        {
            Lit::No();
            continue;
        }
        ll result = imos[r] - imos[l];
        Lit::YesNo(0 < result);
    }
}
