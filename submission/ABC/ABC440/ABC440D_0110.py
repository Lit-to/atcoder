...
def main():
    ...
    d = dict()
    """
    二探で[fr,to)の障害物の個数を数える
    """
    # def countObs(fr:int,to:int):
    #     if (fr,to) in d:
    #         return d[(fr,to)]
    #     left = search(0,N+1,lambda x:A[x]<fr)
    #     right = search(0,N+1,lambda x:A[x]<to)
    #     d[(fr,to)] = right-left
    #     return d[(fr,to)] 

    def eval(v,x,y,left):
        # ある障害物xが区間内にあると仮定したときに、区間の右端がその障害物よりも右側に来るかどうか
        return A[v]<x+y-1+v-left


    for i in range(Q):
        x,y = QUERIES[i]
        # right = search(MAX,0,lambda v:x<=v and v not in obstables)
        left = search(0,N,lambda v:A[v]<x)
        # 二探でどの障害物までが答えとなる区間に含まれるかを調べる
        right = search(left,N,lambda v:eval(v,x,y,left))
        result = x+y-1+right-left
        print(result)
