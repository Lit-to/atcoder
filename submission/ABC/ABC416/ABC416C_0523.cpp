...
namespace LitAlgorithm
{
     ...
     */
    std::vector<ll> GenerateBit(int64_t value, const int64_t digits, ll N)
    {
        std::vector<ll> result(digits);
        for (int64_t i = 0; i < digits; ++i)
        {
            result[digits - i - 1] = value % N;
            value /= N;
        }
        ++value;
        return result;
...
void solve()
{
    ...
    for (ll i = 0; i < mx + 1; ++i)
    {
        vector<ll> bits = LitA::GenerateBit(i, K, N);
        result.push_back("");
        for (ll j = 0; j < K; ++j)
        {
            result.back() += S[bits[j]];
        }
    }
    std::sort(all(result));
