...
int main()
{
    int64_t N;
    std::cin >> N;
    std::vector<int64_t> X(N + 2);
    std::vector<int64_t> P(N + 2);
    N += 2;
    X[0] = INT64_MIN;
    X[N - 1] = INT64_MAX;
    ...
    std::vector<SECTION> SECTIONS(Q);
    for (int64_t i = 0; i < Q; ++i)
    {
        std::cin >> SECTIONS[i].l >> SECTIONS[i].r;
    }
    PSM psm(P);
    for (int64_t i = 0; i < Q; ++i)
    {
        int64_t l = SECTIONS[i].l;
        int64_t r = SECTIONS[i].r;
        int64_t leftIndex = Lit::Search(N - 1, -1, [&](int64_t index)
                                        { return l <= X[index]; });
        int64_t rightIndex = Lit::Search(N - 1, -1, [&](int64_t index)
                                         { return r < X[index]; });
        int64_t value = psm.GetSum(leftIndex, rightIndex);
