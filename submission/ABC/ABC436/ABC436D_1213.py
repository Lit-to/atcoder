...
class Board():
    ...
    def input(f:callable=int)->tuple:
        ...
        board = []
        for i in range(H):
            board.append(list(map(f,input().split()))[0])
...
def main():
    ...
    BOARD,H,W = Board.input(lambda x:split(x,""))
    BOARD.add_wall("#")

    ...

    # 処理スペース
    warps = set(list(LOWER_ALPHABET))
    WARP_POINTS = dict(lambda:[])
    for i in range(H):
        for j in range(W):
            if BOARD[i,j] in warps:
                WARP_POINTS[BOARD[i,j]].append((i,j))
    ...
    while queue:
        cost,pos=queue.deq()
        for moveI,moveJ in LRUD:
            new = pos[0]+moveI,pos[1]+moveJ
            if BOARD[new]!="#":
                if new in done:
                    continue
                if new == goal:
                ...
                queue.enq((cost+1,new))
        if BOARD[pos] in warps:
            warps.remove(BOARD[pos])
            for new in WARP_POINTS[BOARD[pos]]:
                if new == pos:
                    continue
                if new in done:
                    continue
