...
std::vector<char> resolveKakko(std::string &st)
{

    std::vector<char> c_result;
    for (int64_t i = 0; i < st.size(); ++i)
    {
        c_result.push_back(st[i]);
        while (4 <= c_result.size() && c_result[c_result.size() - 4] == '(' && c_result[c_result.size() - 3] == 'x' && c_result[c_result.size() - 2] == 'x' && c_result.back() == ')')
        {
            c_result.pop_back();
            c_result.pop_back();
            c_result.pop_back();
...
void solve()
{
    ...
    std::string B;
    std::cin >> A >> B;
    std::vector<char> A_K_2 = resolveKakko(A);
    std::vector<char> B_K_2 = resolveKakko(B);
    if (A_K_2.size() != B_K_2.size())
    {
        Lit::No();
        return;
    }
    for (int64_t i = 0; i < A_K_2.size(); ++i)
    {
        if (A_K_2[i] == B_K_2[i])
        {
            continue;
