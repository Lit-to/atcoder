...
public:
     ...
     * @brief 縦横でボードを初期化し、各マスをデフォルトコンストラクタ初期化する
     */
    Board(const Board &contents)
        : m_height(contents.GetHeight()), m_width(contents.GetWidth()), m_size(contents.GetSize()), m_data(contents.GetHeight() * contents.GetWidth())
    {
        for (ll i = 0; i < this->GetHeight(); ++i)
        {
            for (ll j = 0; j < this->GetWidth(); ++j)
            {
                (*this)[i, j] = contents[i, j];
            }
        }
    }
...
void solve()
{
    ...
    for (ll i = 0; i < H; ++i)
    {
        for (ll j = 0; j < W; ++j)
        {
            for (auto &dest : LRUD_4)
            {
                ll destY = dest[0] + i;
                ll destX = dest[1] + j;
                if (BOARD.IsOutside(destY, destX))
                {
                    continue;
                }
                if (BOARD[destY, destX] == '#')
                {
                    tasks.push(TASK{.posY = destY, .posX = destX, .cost = 0});
                    break;
                }
            }
        }
    ...
    while (!tasks.empty())
    {
        ...
        // 遷移
        {
            ll destY = dest[0] + task.posY;
            ll destX = dest[1] + task.posX;
            ll index = BOARD.ConvertPosToIndex(destY, destX);
            if (BOARD.IsOutside(destY, destX))
            {
                continue;
            }
            ...
            // タスク処理
            Lit::ChangeMax(result, task.cost + 1);
            // キューの登録
            tasks.push(TASK{.posY = destY, .posX = destX, .cost = task.cost + 1});
        }
    }
    cout << result << endl;
}
