...
seats_appetite=[-1]*N
for i in range(N):
    seats_appetite[i]=(A[i],i+1)
seats_appetite.sort(reverse=False)

seats=[-1]*N
for i in range(N):
    seats[i]=seats_appetite[i][1]

segTree=segmentTree(seats,default=MAX,rule=lambda x,y:min(x,y))
debug(segTree)


for i in B:
    index=search(-1,N,lambda x:seats_appetite[x][0]<=i)
    if index==-1:
        print(-1)
    elif index==0:
        print(seats_appetite[0][1])
    else:
        print(segTree.get_query(1,index+1))
