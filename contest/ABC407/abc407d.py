...
def main():
    ...
    BOARD=Board.input_board_with_wall(H,W,-1,lambda: list(map(int,input().split())))
    # 処理スペース ================================================================================================Lit_to]
    candidate=[]
    for i in range(H):
        for j in range(W):
            for ry,rx in LRUD:
                ...
                if a!=-1 and b!=-1:
                    check=is_collision(a,b)
                    if check[0]:
                        candidate.append((check[1],pos,neigh))
    # candidate.sort(reverse=True)
    result=0
    # debug(*candidate)
    for i in candidate:
        check=is_collision(i[0],result)
        if check[0]:
            result=check[1]
    print(result)
