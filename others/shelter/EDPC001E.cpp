...
int main()
{
    ...
    // int64_t WIDTH = 30;
    //
    int64_t WIDTH = 100001;
    ++W;
    Board<int64_t> DP(N, WIDTH);
    DP.Fill(100000000ll);
    ...
    for (int64_t i = 1; i < N; ++i)
    {
        for (int64_t j = 1; j < WIDTH; ++j)
        {
            ...
            candidates.push_back(INT64_MAX);

            if ((0 <= i - 1) && (j - ITEMS[i].v < 0))
            // 現在の商品の価値を採用したときに残りが採用できない
            {
                candidates.push_back(ITEMS[i].w);
            }
            if ((0 <= i - 1) && (0 <= j - ITEMS[i].v) && (DP[i - 1, j - ITEMS[i].v] + ITEMS[i].w <= W))
            // 現在の商品の価値を採用したときの残りの価値の一番軽いものと現在の商品の重さの和
            {
                candidates.push_back(DP[i - 1, j - ITEMS[i].v] + ITEMS[i].w);
            }
            if (0 <= i - 1)
            // 現在の商品の価値を採用しない
            {
            ...
            DP[i, j] = Min<std::vector<int64_t>>(candidates);
        }
    }
    // std::cerr << DP.Output() << std::endl;
    int64_t value = 0;
    for (int64_t i = WIDTH - 1; 0 <= i; --i)
    {
        if (DP[N - 1, i] <= W - 1)
        {
            value = i;
