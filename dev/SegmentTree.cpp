...
public:
     ...
     * @param eval 評価関数
     */
    SegmentTree(const std::vector<T> &data, const T &identityElement, const std::function<T(T, T)> &eval)
     ...
     * @param eval 評価関数
     */
    SegmentTree(const T &size, const T &identityElement, const std::function<T(T, T)> &eval)
